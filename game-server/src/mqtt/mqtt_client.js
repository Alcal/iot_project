...
function createMqttClient(consumers, publishers, cb) {
  ...
  client.on('connect', function () {
    console.log(`[game-server] Connected to MQTT! Subscribing to prefix ${MQTT_PREFIX}`);
    const topicKeys = Object.keys(consumers);
    client.subscribe(topicKeys, function (err) {
      if (err) {
          console.error('[game-server] Failed to subscribe:', err);
          return;
      }
      console.log(`[game-server] Subscribed to topics: \n ${topicKeys.join('\n\t')}`);
  ...
  client.on('message', function (topic, message) {
    // message is a Buffer
    console.log(`[game-server] MQTT message received on topic '${topic}':`, message.toString());
  ...
  client.on('error', function (err) {
    console.error('[game-server] MQTT client error:', err);
  });

  client.on('close', function (...args) {
    console.log('[game-server] MQTT connection closed', ...args);
  });
